...
func RegisterManageDevicesRoute(fs http.Handler, mux *http.ServeMux) {
	var err error
	manageDevicesTmpl, err = template.ParseFS(ui.FS, "manage_devices.html")
	if err != nil {
		log.Fatalf("Failed to parse templates: %v", err)
	}

	mux.HandleFunc("/manage_devices", handleManageDevices)
}

func handleManageDevices(w http.ResponseWriter, r *http.Request) {
	if err := manageDevicesTmpl.ExecuteTemplate(w, "manage_devices.html", nil); err != nil {
